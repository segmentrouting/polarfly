...
**Status:** DRAFT v0.5 — for internal review
**Date:** August 2026
...
Two flat datacenter topologies now offer credible alternatives to the classic CLOS fat tree: Amazon's RNG [`citation`], a quasi-random expander fabric deployed in production with their homegrown Spraypoint routing protocol and ShuffleBox passive optical cabling; and PolarFly, a deterministic diameter-2 topology built on Erdős–Rényi polarity graphs that asymptotically reaches the Moore bound [`citation`]. The two designs embody opposite philosophies: RNG spends topology (longer paths, statistical guarantees) to keep per-hop routing stateless and running on commodity hardware; PolarFly achieves near-optimal scale and path length but is conventionally held back by lack of shortest-path diversity, odd-prime-power size quantization, and cabling complexity.

This paper argues that SRv6 source routing resolves the objections to structured low-diameter fabrics and proposes **WMP-PolarFly**: a weighted multipath routing design in which SRv6 segment lists and their traffic weights are derived algebraically from the polarity graph's projective-plane coordinates. For any given source-destination pair in the PolarFly fabric, the topology yields one **shortest path (SP)** and a set of **next-shortest-paths (NSPs)** whose count grows with the fabric parameter q; the operator assigns WMP weights across this set (e.g., for q = 7: 40% over the SP and 10% over each of 6 NSPs). We extend the design with multi-plane slicing of high-radix switches, which restores minimal-path redundancy, enables plane-granular hardware heterogeneity, and converts fabric expansion into a purely additive operation. At current 51.2T (512×100G) and forthcoming 102.4T radixes a PolarFly fabric can achieve enormous scale. For example, a 512×100G radix device may be allocated roughly 256×100G for server attachment and 256×100G for fabric attachment¹. The 256×100G fabric ports could map to a 2×q=127 sliced configuration, which creates a dual-plane fabric totaling ~16K ToRs and ~4M server ports at diameter 2 and ~99% Moore-bound efficiency, removing scale as a practical differentiator against RNG.
*`Bruce`*: should we explain what q represents after this phrase "grows with the fabric parameter q"?
...
*`Bruce`*: can we give a brief example of what the math means here? ie, for a 16-port switch q=7, we can build a topology of N = q² + q + 1 or 57 nodes...pretty good for only 8 fabric uplinks
...
**Heterogeneous router degrees.** RNG supports switches with different port counts in a single fabric. If a new-generation switch has 64 ports and the existing fleet has 32, the new switch simply takes more neighbors — the random graph's statistical properties degrade gracefully rather than breaking. In principle, operators can also vary the server-to-fabric port ratio per switch. In practice, the RNG paper does not quantify how path diversity and Spraypoint's load-balancing guarantees degrade as the degree distribution becomes uneven — a heavily lopsided mix (some switches at degree 16, others at degree 64) would weaken expansion properties for the low-degree nodes even though the graph remains connected and routable. WMP-PolarFly is not as flexible as it requires uniform degree (all nodes have the same q value, aka, number of fabric ports allocated) within each plane. The multi-plane slicing described in Section 4 provides a coarser but operationally cleaner heterogeneity model (e.g., a 400G plane alongside a 100G plane).
*`Bruce`*: do you agree with my inserted language in this paragraph?
...
*`Bruce`*: how would we keep the 4 planes truly independent? ie, we're not building 4 separate q=61 polarfly's (16k nodes), we're building one at ~3800 nodes. So essentially each node-to-node connection is actually 4x100G (either link-bundle - not my favorite - or ECMP). If a packet routes across the first 100G (plane-1) in the 4x I arrive at node 2, there's nothing stopping the packet from hopping to plane-2, 3, or 4...unless we're constructing our SRv6 SIDs with uA uSIDs bound to each individual 100G. I think this is more discussion than requirement...ie, maybe some operators don't care about strict plane independence/isolation
...
*`Bruce`*: given my previous comment about planes not being physically separate sets of switches, but logical partitions of a common set of switches, does Plane-granular heterogeneity even apply? or shall we simply say you build it all with the same radix/portspeed and concede the point that RNG has greater flexibility here, but given modern data center builds and commodity hardware, does AWS actually both mixing and matching?
...
*`Bruce`*: this use case is where we get into the scenario of building physically separate / parallel polarflies
...
*`Bruce`*: can we summarize this paragraph and move the details into appendix?
...
*`Bruce`*: do we have any estimates for WMP-PolarFly's cost savings vs. fat tree? Does it get close to RNG's numbers?
...
*`Bruce`*: somewhere in the concluding paragraphs we might reiterate that RNG has its advantages as a design, but its not available to the public, and WMP-PolarFly is :)
...
5. **Mixed-purpose multi-plane configurations** — cross-plane routing for designs that allocate planes to different functions (e.g., internal fabric + DCI egress), including diameter and weight implications.
...
3.  Lakhotia et al., PolarStar
4.  Singla et al., "Jellyfish: Networking Data Centers Randomly," NSDI 2012
5.  Valadarsky et al., "Xpander," 2016
6.  Besta & Hoefler, "Slim Fly," SC14
7.  OpenAI et al., MRC specification, OCP, May 2026
8.  OpenAI et al., "Resilient AI Supercomputer Networking using MRC and SRv6"
9.  Zhou et al., "WCMP: Weighted Cost Multipathing," EuroSys 2014
10. RFC 9256 (SR Policy Architecture)
11. RFC 8986 (SRv6 Network Programming)
12. Erdős–Rényi polarity graph / Brown graph literature
13. EvalNet toolchain, arXiv:2105.12663.
...
**v0.4 → v0.5:**
- Replaced "regimes" with "deployments" / "deployment types" throughout
- Section 3.1: leads with q=61 (~703K entries, exceeding TH5 300K ALPM) as realistic production-scale example; q=127 (6.2M entries) retained as scale-ceiling illustration
- Section 3.3: added high-q weight convergence discussion (q=127 approaches near-uniform/ECMP); added paragraph on on-demand segment-list computation at NIC (no pre-populated FIB to all destinations)
- Section 3.4: added parenthetical clarifying RNG still requires routable addressing; misconfiguration immunity is about per-path state, not IP assignment
- Section 4 table: dropped q=509; made q=127 dual-option (512-port dual-plane / 102.4T quad-plane); q=251 as dual-plane on 102.4T
- Section 4.1: added cloud fan-in/incast parenthetical in 4×q=61 description; added 8×q=31 option for AI backend with verified math (~254K server ports)
- Section 4.2: added DCI/egress plane as future-work item; stripped patent disclosure language
- Section 7.3: new paragraph on SP/NSP latency differential and MRC reorder tolerance (1–2μs gap well within MRC's window; NSCC adapts per-path; WMP priors and NSCC reinforce each other)
- Section 8: wove in host-based policy execution trend in conclusion paragraph; replaced all "regime" language with "deployment" variants
- Section 9: added item 5 on mixed-purpose multi-plane configurations
- Stripped all remaining review annotations
...
- Section 4.2: renamed k→p for plane count to avoid collision with k-shortest-path; added plain-language intersection explanation ("shared language" analogy); concrete 3×q=83 example
...
- Section 8: added Advantage column to scorecard; replaced "retires" with "addresses"; updated closing SRv6 observation
